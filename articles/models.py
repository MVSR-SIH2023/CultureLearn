...
class Podcast(models.Model):
    ...
    audio = models.FileField(upload_to="podcast_audios",validators=[FileExtensionValidator(['mp3']), validate_audio_file_extension])
    ...
    def save(self, *args, **kwargs):
        if not self.pk:  # Only if it's a new object
            last_podcast = Podcast.objects.all().order_by('-order').first()
            if last_podcast:
                self.order = last_podcast.order + 1
            else:
                self.order = 1  # Set the initial order
