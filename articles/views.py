...
def upload_single_podcast(request):
    ...
    if request.method=="POST":
        ...
        name = request.POST.get('name')
        image = request.FILES.get("templates")
        hindi_audio = request.FILES.get("hindi_audio")
        english_audio = request.FILES.get("english_audio")
        telugu_audio = request.FILES.get("telugu_audio")
        description = request.POST.get("content")
        ...
        hindi_audio.name = str(p.id)
        image.name =str(p.id)
        p.hindi_audio = hindi_audio
        if english_audio:
            p.english_audio = english_audio
        if telugu_audio:
            p.telugu_audio = telugu_audio
        p.image =image
        p.save()
